...
struct ProfileView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .alert("保存成功", isPresented: $showSaveSuccess) {
                Button("好的", role: .cancel) { viewModel.resetState() }
            } message: {
                Text("个人资料已更新")
            }
            ...
            .onAppear { bioText = appState.bio }
            .onChange(of: viewModel.loginState) { _, newState in
                if case .success = newState { showSaveSuccess = true }
                else if case .error = newState { showError = true }
            }
        }
    }
    ...
    private var vipBadgeLabel: some View {
        HStack(spacing: 4) {
            ...
            ZStack(alignment: .topLeading) {
                ...
                TextEditor(text: $bioText)
                    ...
                    .onChange(of: appState.bio) { _, newValue in
                        if bioText.isEmpty { bioText = newValue }
                    }
            }
            .padding(8)
            .background(
    ...
    private var saveButton: some View {
        let isTextEmpty = bioText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        return Button {
            saveProfile()
        } label: {
            HStack {
                ...
                Text("保存修改").font(.subheadline.bold())
            }
            ...
            .background(
                RoundedRectangle(cornerRadius: 12)
                    .fill(isTextEmpty ? Color.appTextHint : Color.appPrimary)
            )
        }
        .disabled(viewModel.loginState == .loading || isTextEmpty)
        .padding(.horizontal, 16)
    }
